...
function App()
{
    ...
    async function viewComplaints(event){
        ...
        try{
            ...
            // console.log(complaintsModalContent.length)
            } catch (error) {
                console.error("Error: ",error)
            }      
        }

    async function resolveComplaints(id){
        try{
            const response = await fetch('http://localhost:5000/api/mapDept/resolveComplaints',{
                method:'POST',
                headers:{
                    'Content-Type':'application/json'
                },
            ...
            <Modal isOpen={complaintsModal} onClose={() => setComplaintsModal(false)} blockScrollOnMount={false}>
                <ModalOverlay />
                    ...
                    <p key={index} style={{marginTop:13}}> 
                        ...
                        <button style={{marginLeft:680,width:100,fontSize:15,background:'skyblue',border:'none',borderRadius:'7px'}} onClick={()=>resolveComplaints(complaint.ID)}>Resolve</button>
                    ...
                    {complaintsModalContent.data=='No data' &&  (
                        <div>
                        <br/>
                        <h1 style={{textAlign:'center'}}>------NO NEW COMPLAINTS------</h1>
                        <br/>
                        </div>
                    )}
                    </ModalBody>
